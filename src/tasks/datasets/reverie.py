...
class REVERIEDataset(MP3DDataset):
    ...
    def __init__(
        ...
        self.multi_endpoints = config.dataset.REVERIE.multi_endpoints

    def preprocess_item(self, item):
        if self.split!="train" or "end_vps" not in item or (not self.multi_endpoints):
            return item
        ...
        item["path"] = self.environments[simulator_name]["shortest_paths"][item["scan"]][start_vp][end_vp]
        return item
    ...
    def save_json(self, results, path, item_metrics=None):    
        ...
        for item in results:
            ...
            item['predObjId'] = int(item['pred_objid']) if item.get('pred_objid') is not None else 0
