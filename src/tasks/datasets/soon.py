...
class SOONDataset(MP3DDataset):
        ...
        if self.training:
            ...
            if self.multi_endpoints:
                for i, one_item in enumerate(batch):
                    ...
                    end_vps[i] = end_vp
            for i, one_item in enumerate(batch):
                one_item['path'] = self.environments[simulator_name]["shortest_paths"][one_item['scan']][start_vps[i]][end_vps[i]]
            item = batch[0]
        else:
            item['heading'] = 1.52
        item['elevation'] = 0
    ...
    def eval_metrics(self, preds, logger, name):
        ...
        for item in preds:
            ...
            obj_heading = item['pred_obj_direction'][0] if item.get('pred_obj_direction') is not None else None
            obj_elevation = item['pred_obj_direction'][1] if item.get('pred_obj_direction') is not None else None
        ...
        for item in results:
            ...
            new_item = {
                ...
                'trajectory':[{
                    ...
                    "obj_heading": [item['pred_obj_direction'][0] if item.get('pred_obj_direction') is not None else 0],
                    "obj_elevation": [item['pred_obj_direction'][1] if item.get('pred_obj_direction') is not None else 0]
                }]
            }
