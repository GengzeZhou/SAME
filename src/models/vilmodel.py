...
class BertOutAttention(nn.Module):
    def __init__(self, config, ctx_dim=None):
        ...
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)
        self.all_head_size = self.num_attention_heads * self.attention_head_size
        self.moe_position = config.moe_position
    ...
    def forward(self, hidden_states, context, attention_mask=None, router_weights=None, selected_experts=None):
        if router_weights is not None and self.moe_position == "Attn_q":
            mixed_query_layer = self.query(hidden_states, router_weights, selected_experts)
        else:
            mixed_query_layer = self.query(hidden_states)
        
        if router_weights is not None and self.moe_position == "Attn_kv":
            mixed_key_layer = self.key(context, router_weights, selected_experts)
            mixed_value_layer = self.value(context, router_weights, selected_experts)
        else:
...
class BertXAttention(nn.Module):
    ...
    def forward(self, input_tensor, ctx_tensor, ctx_att_mask=None, router_weights=None, selected_experts=None):
        if router_weights is not None:
            output, attention_scores = self.att(input_tensor, ctx_tensor, ctx_att_mask, router_weights, selected_experts)
        else:
...
class GraphLXRTXLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.use_moe_layer = config.use_moe_layer
        self.moe_position = config.moe_position
        ...
        if self.use_moe_layer and self.moe_position == "Attn_q":
            self.visual_attention.att.query = TaskMoeBlock(config)
        elif self.use_moe_layer and self.moe_position == "Attn_kv":
            self.visual_attention.att.key = TaskMoeBlock(config)
            self.visual_attention.att.value = TaskMoeBlock(config)
        elif self.use_moe_layer and self.moe_position == "FFN":
            if self.moe_type == "Sparse":
                self.moe = SparseMoeBlock(config)
            elif self.moe_type == "Task":
                self.moe = TaskMoeBlock(config)
            del self.visn_inter
            del self.visn_output

    def forward(
        self, lang_feats, lang_attention_mask, visn_feats, visn_attention_mask,
        graph_sprels=None, router_weights=None, selected_experts=None
    ): 
        # Cross Attention
        if self.use_moe_layer and (self.moe_position == "Attn_q" or self.moe_position == "Attn_kv"):
            visn_att_output = self.visual_attention(
                visn_feats, lang_feats, ctx_att_mask=lang_attention_mask,
                router_weights=router_weights, selected_experts=selected_experts
            )[0]
        else: 
            visn_att_output = self.visual_attention(
                visn_feats, lang_feats, ctx_att_mask=lang_attention_mask
        ...
        # FFN
        router_logits = None
        if self.use_moe_layer and self.moe_position == "FFN":
            if self.moe_type == "Sparse":
                visn_output, router_logits = self.moe(visn_att_output)
            elif self.moe_type == "Task":
                visn_output = self.moe(visn_att_output, router_weights, selected_experts)
        else:
            visn_inter_output = self.visn_inter(visn_att_output)
            visn_output = self.visn_output(visn_inter_output, visn_att_output)

        return (visn_output, router_logits)
...
class CrossmodalEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.use_moe_layer = config.use_moe_layer
        self.moe_type = config.moe_type
        self.task_routing_feature = config.task_routing_feature
        if self.use_moe_layer and config.moe_type == "Task":
            if "multi" in self.task_routing_feature:
                self.routing_embed_merge = nn.Linear(config.hidden_size * 2, config.hidden_size)
            if "task_id" in self.task_routing_feature:
                self.task_embedding = nn.Embedding(config.num_tasks, config.hidden_size)
            self.router = TaskMoeRouting(config)

    def forward(self, txt_embeds, txt_masks, img_embeds, img_masks, graph_sprels=None, **kwargs):
        extended_txt_masks = extend_neg_masks(txt_masks)
        extended_img_masks = extend_neg_masks(img_masks) # (N, 1(H), 1(L_q), L_v)

        if self.use_moe_layer and self.moe_type == "Task":
            # Routing embeddings
            routing_embeds = None
            if self.task_routing_feature == "task_id":
                task = kwargs["task"]
                task_embeds = self.task_embedding(task)
                routing_embeds = task_embeds
            elif self.task_routing_feature == "mean":
                routing_embeds = (txt_embeds * txt_masks.unsqueeze(-1)).mean(dim=1)
            elif self.task_routing_feature == "cls":
                routing_embeds = txt_embeds[:, 0]
            elif self.task_routing_feature == "task_id_cls":
                # add task embeddings with text cls embeddings
                task = kwargs["task"]
                task_embeds = self.task_embedding(task)
                routing_embeds = txt_embeds[:, 0] + task_embeds
            else:
                # multi: add task embeddings with mean image embeddings
                mean_img_embeds = (img_embeds * img_masks.unsqueeze(-1)).mean(dim=1)
                text_cls_embeds = txt_embeds[:, 0]
                merged_embeds = torch.cat([mean_img_embeds, text_cls_embeds], dim=-1)
                merged_embeds = self.routing_embed_merge(merged_embeds)
                if self.task_routing_feature == "task_id_multi":
                    task = kwargs["task"]
                    task_embeds = self.task_embedding(task)
                    routing_embeds = merged_embeds + task_embeds
                else:
                    routing_embeds = merged_embeds
            
            router_weights, selected_experts, router_logits = self.router(routing_embeds, txt_masks)

        all_router_logits = () if self.use_moe_layer else None
        # For task routing, only collect router logits from input text embeddings
        if self.use_moe_layer and self.moe_type == "Task":
            all_router_logits += (router_logits,)
        for layer_module in self.x_layers:
            # Forward TaskMoeRouting to the next crossmodal layer
            if self.use_moe_layer and self.moe_type == "Task":
            ...
            # Forward SparseMoeBlock or standard BertAttentionBlock to the next crossmodal layer
            else:
                outputs = layer_module(
                    txt_embeds, extended_txt_masks, 
                    img_embeds, extended_img_masks,
            ...
            # For task routing, collect router logits from each crossmodal layer
            if self.use_moe_layer and self.moe_type == "Sparse":
                all_router_logits += (outputs[1],)
        
        # # Calculate routing balance loss
        # aux_loss = None
        # if self.use_moe_layer:
        #     aux_loss = load_balancing_loss_func(
        #         all_router_logits, self.num_experts, self.num_experts_per_tok
        #     )

        # return (img_embeds, aux_loss)
        return (img_embeds, all_router_logits)
...
class ImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        if config.enable_og and config.obj_feat_size != config.image_feat_size:
...
class LocalVPEncoder(nn.Module):
    ...
    def forward(
        ...
        vp_embeds, all_router_logits = self.encoder(txt_embeds, txt_masks, vp_embeds, vp_masks)
        return vp_embeds, all_router_logits
...
class GlobalMapEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.gmap_step_embeddings = nn.Embedding(config.gmap_max_action_steps, config.hidden_size)
        self.encoder = CrossmodalEncoder(config)
    ...
    def forward(
        ...
        gmap_embeds, all_router_logits = self.encoder(
            txt_embeds, txt_masks, gmap_embeds, gmap_masks,
            graph_sprels=graph_sprels
        )
        return gmap_embeds, all_router_logits
...
class BLockSparseTop2Linear(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.w = nn.Linear(self.hidden_size, self.all_head_size)
...
class BLockSparseTop2BertSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, hidden_states, input_tensor, routing_weights):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return routing_weights * hidden_states

class BLockSparseTop2BertMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.ffn_dim = config.intermediate_size
        self.hidden_dim = config.hidden_size
    ...
    def forward(self, hidden_states, routing_weights):
        current_hidden_states = self.inter(hidden_states)
        current_hidden_states = self.output(current_hidden_states, hidden_states)
        return routing_weights * current_hidden_states
...
class SparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList([BLockSparseTop2BertMLP(config) for _ in range(self.num_experts)])
...
class TaskMoeBlock(nn.Module):
    """
    MoE block for task routing.
    It takes the expert logits and the hidden states and computes the
    output hidden states from the experts.
    """
    def __init__(self, config):
        super().__init__()
        self.hidden_dim = config.hidden_size
        ...
        self.num_experts = config.num_experts
        self.top_k = config.num_experts_per_tok
        if config.moe_position == "FFN":
            self.experts = nn.ModuleList([BLockSparseTop2BertMLP(config) for _ in range(self.num_experts)])
        else:
            self.experts = nn.ModuleList([BLockSparseTop2Linear(config) for _ in range(self.num_experts)])
    ...
    def forward(self, hidden_states: torch.Tensor, routing_weights: torch.Tensor, selected_experts: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            expert_layer = self.experts[expert_idx]
            # sample_idx: (num_samples, ), the indices of the samples that are assigned to the current expert
            # routing_weights_idx: (num_samples, ), the indices of the routing weight of the idx_sample for the current expert
            sample_idx, routing_weights_idx = torch.where(selected_experts == expert_idx)

            if routing_weights_idx.shape[0] == 0:
                continue

            # in torch it is faster to index using lists than torch tensors
            sample_idx_list = sample_idx.tolist()
            routing_weights_idx_list = routing_weights_idx.tolist()

            # Index the correct hidden states and compute the expert hidden state for
            # the current expert. We need to make sure to multiply the output hidden
            # states by `routing_weights` on the corresponding tokens (top-1 and top-2)
            # current_state: (num_samples, seq_len, hidden_dim)
            # current_routing_weights: (num_samples, 1, 1)
            current_state = hidden_states[sample_idx_list]
            current_routing_weights = routing_weights[sample_idx_list, routing_weights_idx_list].reshape(-1, 1, 1)
            current_hidden_states = expert_layer(current_state, current_routing_weights)

            # However `index_add_` only support torch tensors for indexing so we'll use
            # the `sample_idx` tensor here.
            # final_hidden_states: (batch_size, seq_len, hidden_dim)
            # final_hidden_states[sample_idx_list] += current_hidden_states.to(hidden_states.dtype)
            final_hidden_states.index_add_(0, sample_idx, current_hidden_states.to(hidden_states.dtype))
        assert final_hidden_states.shape == (batch_size, sequence_length, hidden_dim)
        return final_hidden_states

class TaskMoeRouting(nn.Module):
    """
    Task routing conditions on the language instructions.
    """
    def __init__(self, config):
        super().__init__()
        self.hidden_dim = config.hidden_size
    ...
    def forward(self, embeddings: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            embeddings: 
                if task_routing_feature == "mean", embeddings is the mean of the text embeddings
                if task_routing_feature == "cls", embeddings is the text [CLS] token embedding
                if task_routing_feature == "task_id", embeddings is the task embedding
                if task_routing_feature == "task_id_cls", embeddings is the addition of the task embedding and the text [CLS] token embedding
                if task_routing_feature == "multi", embeddings is the addition of the task embedding and the merged text and visual embeddings
            mask: (batch, seq_len)
        """
        # router_logits: (batch, n_experts)
        router_logits = self.gate(embeddings)

        routing_weights = F.softmax(router_logits, dim=0, dtype=torch.float)
        routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=-1)
        routing_weights /= routing_weights.sum(dim=-1, keepdim=True)
        # we cast back to the input dtype
        routing_weights = routing_weights.to(embeddings.dtype)
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if config.load_text_features:
            self.lang_encoder = nn.Linear(config.text_feat_size, config.hidden_size)
        else:
    ...
    def forward_text(self, txt_ids, txt_masks):
        if self.config.load_text_features:
            txt_embeds = self.lang_encoder(txt_ids)
        else:
    ...
    def forward_navigation_per_step(
        ...
        gmap_embeds, glo_all_router_logits = self.global_encoder.encoder(
            txt_embeds, txt_masks, gmap_embeds, gmap_masks,
            graph_sprels=graph_sprels,
        ...
        vp_embeds, loc_all_router_logits = self.local_encoder.encoder(txt_embeds, txt_masks, vp_embeds, vp_masks, task=navigation_task)

        all_router_logits = None
        if glo_all_router_logits is not None:
            all_router_logits = glo_all_router_logits + loc_all_router_logits
